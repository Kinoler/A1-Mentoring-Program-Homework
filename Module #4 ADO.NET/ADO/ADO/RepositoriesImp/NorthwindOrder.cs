...
namespace ADO.RepositoriesImp
{
    public class NorthwindOrder : INorthwindTable<Order>
    {
        ...
        internal const string AddQuery =
            @"INSERT INTO Orders 
        ...
        internal const string DeleteOneQuery =
            @"DELETE FROM Orders WHERE OrderID = {0};";

        internal const string UpdateQuery =
            @"UPDATE * FROM Orders
            SET 
            CustomerID = {1},
            EmployeeID = {2},
        ...
        {
            var orderToDelete = GetElement(id);

            if (orderToDelete.Status == OrderState.Complete)
                throw new ArgumentException($"The order with status {OrderState.Complete}, can not be deleted", nameof(orderToDelete));
